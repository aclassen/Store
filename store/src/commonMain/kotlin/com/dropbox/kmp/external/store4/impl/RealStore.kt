...
import org.burnoutcrew.cachekmp.cacheBuilder
...
internal class RealStore<Key : Any, Input : Any, Output : Any>(
    ...
    private val memCache = memoryPolicy?.let {
        cacheBuilder<Key, Output> {
            if (memoryPolicy.hasAccessPolicy) expireAfterAccess { memoryPolicy.expireAfterAccess }
            if (memoryPolicy.hasWritePolicy) expireAfterWrite { memoryPolicy.expireAfterWrite }
            if (memoryPolicy.hasMaxSize) maximumSize { memoryPolicy.maxSize }
