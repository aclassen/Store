 ...
 */
package org.burnoutcrew.cachekmp
...
internal class LocalCache<K : Any, V : Any>(builder: CacheBuilder<K, V>) {
    ...
    fun evictsBySize(): Boolean {
        return maxWeight >= 0
    }

    fun customWeigher(): Boolean {
        return weigher !== OneWeigher
    }

    private fun expiresAfterWrite(): Boolean {
        return expireAfterWriteNanos > 0
    }

    private fun expiresAfterAccess(): Boolean {
        return expireAfterAccessNanos > 0
    }

    fun usesAccessQueue(): Boolean {
        return expiresAfterAccess() || evictsBySize()
    }

    fun usesWriteQueue(): Boolean {
        return expiresAfterWrite()
    }

    fun recordsWrite(): Boolean {
        return expiresAfterWrite()
    }

    fun recordsAccess(): Boolean {
        return expiresAfterAccess()
    }

    private fun recordsTime(): Boolean {
        return recordsWrite() || recordsAccess()
    }

    private fun usesWriteEntries(): Boolean {
        return usesWriteQueue() || recordsWrite()
    }

    private fun usesAccessEntries(): Boolean {
     ...
     */
    internal interface ValueReference<K : Any, V : Any> {
        /**
     ...
     */
    internal interface ReferenceEntry<K : Any, V : Any> {
        /**
     ...
     */
    internal open class StrongEntry<K : Any, V : Any>(
    ...
    internal class StrongAccessEntry<K : Any, V : Any>(key: K, hash: Int, next: ReferenceEntry<K, V>?) :
    ...
    internal class StrongWriteEntry<K : Any, V : Any>(key: K, hash: Int, next: ReferenceEntry<K, V>?) :
        StrongEntry<K, V>(key, hash, next) {
    ...
    internal class StrongAccessWriteEntry<K : Any, V : Any>(key: K, hash: Int, next: ReferenceEntry<K, V>?) :
        StrongEntry<K, V>(key, hash, next) {
        // The code below is exactly the same for each access entry type.
        private val _accessTime = atomic(Long.MAX_VALUE)
        override var accessTime: Long by _accessTime
     ...
     */
    internal open class StrongValueReference<K : Any, V : Any>(private val referent: V) : ValueReference<K, V> {
     ...
     * References a strong value.
     */
    internal class WeightedStrongValueReference<K : Any, V : Any>(referent: V, override val weight: Int) :
     ...
     */
    // Guarded By Segment.this
    fun newValueReference(entry: ReferenceEntry<K, V>, value: V, weight: Int): ValueReference<K, V> {
        val hash = entry.hash
    ...
    private fun segmentFor(hash: Int): Segment<K, V> {
        ...
        return segments[hash ushr segmentShift and segmentMask] as Segment<K, V>
    }
    ...
    private fun createSegment(
        ...
        return Segment(this, initialCapacity, maxSegmentWeight)
    }
    // expiration
    /**
     * Returns true if the entry has expired.
     */
    fun isExpired(entry: ReferenceEntry<K, V>, now: Long): Boolean {
        if (expiresAfterAccess()
            && now - entry.accessTime >= expireAfterAccessNanos
        ) {
            return true
        }
        return (expiresAfterWrite() && now - entry.writeTime >= expireAfterWriteNanos)
    }

    private fun newSegmentArray(ssize: Int): Array<Segment<K, V>?> {
        return arrayOfNulls(ssize)
    }
    // Inner Classes
    /**
    ...
    internal class Segment<K : Any, V : Any>(
        ...
        internal val reentrantLock = reentrantLock()
        ...
        fun copyEntry(original: ReferenceEntry<K, V>, newNext: ReferenceEntry<K, V>?): ReferenceEntry<K, V>? {
            ...
            val newEntry: ReferenceEntry<K, V> = map.entryFactory.copyEntry(this, original, newNext)
        ...
        fun setValue(entry: ReferenceEntry<K, V>, key: K, value: V, now: Long) {
            ...
            val valueReference: ValueReference<K, V> = map.valueStrength.referenceValue(this, entry, value, weight)
    ...
    internal class WriteQueue<K : Any, V : Any> : MutableQueue<ReferenceEntry<K, V>> {
     ...
     * for the current model.
     */
    internal class AccessQueue<K : Any, V : Any> : MutableQueue<ReferenceEntry<K, V>> {
        private val head: ReferenceEntry<K, V> = object : ReferenceEntry<K, V> {
            override var accessTime: Long
                get() = Long.MAX_VALUE
                set(_) {}
     ...
     */
        ...
        fun <K : Any, V : Any> unset(): ValueReference<K, V> {
            return UNSET as ValueReference<K, V>
        }

        @Suppress("UNCHECKED_CAST")
        fun <K : Any, V : Any> nullEntry(): ReferenceEntry<K, V> {
        ...
        private fun <E : Any> discardingQueue(): MutableQueue<E> {
            return DISCARDING_QUEUE as MutableQueue<E>
        }
        ...
        // queues
        // Guarded By Segment.this
        fun <K : Any, V : Any> connectAccessOrder(previous: ReferenceEntry<K, V>, next: ReferenceEntry<K, V>) {
            previous.nextInAccessQueue = next
            next.previousInAccessQueue = previous
        }

        // Guarded By Segment.this
        fun <K : Any, V : Any> nullifyAccessOrder(nulled: ReferenceEntry<K, V>) {
        ...
        // Guarded By Segment.this
        fun <K : Any, V : Any> connectWriteOrder(previous: ReferenceEntry<K, V>, next: ReferenceEntry<K, V>) {
            previous.nextInWriteQueue = next
            next.previousInWriteQueue = previous
        }

        // Guarded By Segment.this
        fun <K : Any, V : Any> nullifyWriteOrder(nulled: ReferenceEntry<K, V>) {
            val nullEntry: ReferenceEntry<K, V> = nullEntry()
